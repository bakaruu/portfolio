...
void main() {
  ...
  float grain = snoise(vec2(x * 300.0, y * 300.0) + time * 30.0) * 0.55;
  float scan  = sin(y * resolution.y * 1.5) * 0.18;
  float vignette = 1.0 - smoothstep(0.3, 1.0, length(uv - 0.5) * 1.6);

  float brightness = max(0.0, grain + scan) * vignette;
  float c = mix(brightness, 1.0 - brightness, invert);
...
export default function NavBadTV({ light }: { light: boolean }) {
  ...
  useEffect(() => {
    ...
    const uInvert = gl.getUniformLocation(prog, "invert");
    ...
    const tick = () => {
      ...
      gl.uniform1f(uInvert, lightRef.current ? 1.0 : 0.0);
